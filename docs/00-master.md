...
- Expo Router, TanStack Query(서버상태), Zustand(클라상태), RHF+Zod(폼), **자체 경량 디자인 시스템**(토큰+`components/ui/*`, 2026-07 리뉴얼 때 gluestack-ui/NativeWind에서 전환), date-fns(+tz)

### 7-2. 네비게이션
- 모바일: 하단 탭바 / 웹: 사이드바 — 동일 5섹션
- [일정] [가계부] [기록] [홈/타임라인] [더보기] (2026-07 리뉴얼 — 사용빈도 기준 재배열. 통합 타임라인은 여전히 핵심 차별점 화면이나 탭 위치만 조정)

### 7-3. 화면 (MVP ~25개)
- 인증(3), 홈/타임라인(2), 가계부(6), 일정(3), 일상기록(4, 앨범뷰는 미구현·추후), 묶음(2), 가족/설정(5 — 비밀번호 변경 추가)
...
- 거래·일정 입력은 우측 슬라이드 사이드패널. 기록은 클릭 시 읽기전용 상세 → "수정"으로 편집 전환
- 가계부/일정/기록 화면: 인라인·캘린더 소형 토글(기본 캘린더) + 인라인 정렬(asc/desc, 화면 로컬), 캘린더는 다일 항목을 구글캘린더식 스팬 막대로 표시
- 묶음(Collection)은 일정 작성에서 생성(태그 부여 가능) → 가계부 다중선택/기록 화면에서 기존 묶음에 연결. 묶음 요약 화면에서 연결된 거래·기록 본문까지 통합 조회
- 일정·기록 화면에 전체기간 검색(제목/설명·본문/태그) — 캘린더 모드에서 결과 선택 시 해당 날짜로 이동
...
### V1.5
- 통합 검색 (일정/가계부/기록/사진 횡단) — 2026-07 리뉴얼에서 일정·기록 개별 화면의 전체기간 텍스트 검색(제목/설명·본문/태그)까지는 선구현. 가계부·사진까지 아우르는 단일 통합 검색은 여전히 V1.5 범위.
